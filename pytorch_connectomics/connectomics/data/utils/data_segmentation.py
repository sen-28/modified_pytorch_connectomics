...
def reduce_label(seg, do_type=False):
    ...
    uid = np.unique(seg)
    # print("UID in reduce_label: {}".format(uid))
...
def generate_probability_map_at_center(gauss_size, sigma):
    """Generates a probability map with a Gaussian blob centered at the image's center"""
    # Calculate the center coordinate of the image
    center_coord = tuple(np.array(gauss_size) // 2)

    # Create a Gaussian kernel centered at the image's center
    kernel = np.zeros(gauss_size)
    kernel[center_coord] = 1
    kernel = gaussian_filter(kernel, sigma, mode='constant')
    # kernel[center_coord] = 1
    # Normalize the probability map to [0, 1]
    kernel = (kernel - np.min(kernel)) / (np.max(kernel) - np.min(kernel))
    return kernel

def calculate_gaussian_width(sigma):
    width = 2 * np.ceil(2.3548 * sigma)
    return int(width)

def seg2gauss(label, topt):
    # generate gauss shape
    topt_list = [num for num in topt.split('-')]
    sig = int(topt_list[1])
    gaussian_width = calculate_gaussian_width(sig)
    # print("Gaussian Width:", gaussian_width)
    gauss_shape = (gaussian_width,gaussian_width,gaussian_width)

    img_size = label.shape

    pre_indices = np.where(label == 1)
    pre_coordinates = list(zip(pre_indices[0], pre_indices[1], pre_indices[2]))
      
    gauss = generate_probability_map_at_center(gauss_shape,sig)

    prob_map = np.zeros(img_size)

    # Iterate over the set of special coordinates and superimpose with gauss
    for coord in pre_coordinates:
        i, j, k = coord
        # Determine the range of indices in the larger volume to replace with the smaller volume
        i_start = max(i, 0)
        j_start = max(j, 0)
        k_start = max(k, 0)
        i_end = min(i+gauss.shape[0], prob_map.shape[0])
        j_end = min(j+gauss.shape[1], prob_map.shape[1])
        k_end = min(k+gauss.shape[2], prob_map.shape[2])
        # Replace values in the larger volume within the range of indices with the corresponding values in the smaller volume
        prob_map[i_start:i_end, j_start:j_end, k_start:k_end] += gauss[:i_end-i_start, :j_end-j_start, :k_end-k_start]
...
def seg_to_targets(
    ...
    for tid, topt in enumerate(topts):
        ...
        elif topt[0] == 'A': #generate gaussian blobs for point masks
            gauss_targ = seg2gauss(label,topt)
            out[tid] = gauss_targ[np.newaxis, :].astype(np.float32)
        else:
